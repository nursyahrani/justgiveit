 ...
 * Signup form
 */
class NotificationBidReplyForm extends Model
{
    //url _key value
    public $bid_id;
    ...
    const NOTIFICATION_VERB_NAME = "bid_reply";
    ...
    public function rules()
    {
        return [
            ...
            [['bid_id', 'new_actor_id'], 'required']
        ];
    }
    
    public function create() {
        $this->bid = Bid::find()->where(['bid_id' => $this->bid_id])->one();
        $this->post = Post::find()->where(['stuff_id' => $this->bid['stuff_id']])->one();
        ...
        //bid owner
        if($this->bid['proposer_id'] === $this->new_actor_id){
            $receiver_id = $this->post['poster_id'];
        } else {
            $receiver_id = $this->bid['proposer_id'];
        }
        
        $notification_receiver = $this->getNotificationReceiver($notification->notification_id, $receiver_id);
        if(!$notification_receiver) {
            ...
            $notification_receiver->receiver_id = $receiver_id;
